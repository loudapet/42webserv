...
/*   Updated: 2024/06/20 18:44:19 by okraus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	HttpRequest::validateResourceAccess(const Location& location)
{
	...
	std::cout << CLR2 << pos << " " << root << "Final path:\t" << this->targetResource << RESET << std::endl;
   	this->targetResource.replace(pos, path.length(), root);
	std::cout << CLR2 << "Final path:\t" << this->targetResource << RESET << std::endl;
	this->allowedDirListing = location.getAutoindex();
	// this->targetResource = root + path + this->requestLine.requestTarget.absolutePath;
	...
	if (!this->isRedirect && this->requestLine.method == "GET")
	{
		...
		std::cout << CLR1 << "Final path:\t" << this->targetResource << RESET << std::endl;
			std::cout << CLR1 << validFile << RESET << std::endl;
		if (validFile < 0)
			throw (ResponseException(404, "File does not exist"));
		std::cout << CLR1 << "NOOOOO" << RESET << std::endl;
