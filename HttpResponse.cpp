...
/*   Updated: 2024/06/20 18:42:24 by okraus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	HttpResponse::readDirectoryListing(const std::string& targetResource)
{
	...
	for (dirent* dir = readdir(dirPtr); dir != NULL; dir = readdir(dirPtr))
	{
		...
		tm *curr_tm = std::gmtime(&(fileCheckBuff.st_mtim.tv_sec)); //different in mac?
		char time[100];
		...
		else if (dir->d_type == DT_REG)
		{
			...
			<< std::left << std::setw(30) << time
			<< " "
			<< std::right << std::setw(20) << size << strsize
			<< "\r\n";
		}
